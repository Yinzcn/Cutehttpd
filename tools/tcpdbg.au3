...
$C_Port = GUICtrlCreateInput("8080", 274, 16, 65, 20)
...
$C_Conn = GUICtrlCreateButton("Connect", 632, 14, 64, 25)
...
$C_Send = GUICtrlCreateButton("Send", 632, 72, 65, 25)
...
Func SocketConnect()
  GUICtrlSetState($C_Conn, $GUI_DISABLE)
  TCPStartup()
  $IP = _GUICtrlIpAddress_Get($IPAddr)
  $Port = GUICtrlRead($C_Port)
  $Socket = TCPConnect($IP, $Port)
  If $Socket > 0 Then
    GUICtrlSetData($C_Conn, "Disconn")
    GUICtrlSetState($C_Conn, $GUI_ENABLE)
    GUICtrlSetState($C_Send, $GUI_ENABLE)
  Else
    If @error == 1 Then
...
Func SocketClose()
  TCPCloseSocket($Socket)
  TCPShutdown()
  GUICtrlSetData($C_Conn, "Connect")
  ;GUICtrlSetState($C_Send, $GUI_DISABLE)
  $Socket = 0
EndFunc
